...
// @match        http://*/*
// @match        https://*/*
// @exclude-match https://feedly.com/i/*
...
// ==/UserScript==
// 24.1210.1053      添加支持Follow
// 2024.02.21.1004   替换js的cdn加速地址
...
const en = {
    ...
    "use_follow": "Use Follow Subscription",
...
setTimeout(function () {
    ...
    GM_registerMenuCommand(lang.use_follow, function () {
        GM_setValue("rss_service", "follow");
    });
...
function addRSS(title, link) {
    ...
    if (Object.keys(feedsMap).length == 0) {
        ...
        var rssPlusDiv = document.getElementById("rss-plus")
        if (!rssPlusDiv){
            rssPlusDiv = document.createElement("div");
            rssPlusDiv.setAttribute("id", "rss-plus");
            body.appendChild(rssPlusDiv);
        }
        var rssPlusIFrame = document.getElementById("rss-plus-iframe")
        if (!rssPlusIFrame){
            rssPlusIFrame = document.createElement("iframe");
        ...
        var styleEl = rssPlusDocument.getElementById("rss-plus-style")
        if (!styleEl){
            styleEl = document.createElement("style");
            styleEl.setAttribute("id", "rss-plus-style");
            styleEl.innerHTML = BOX_STYLE;
            frameBody.appendChild(styleEl);
        }

        var rssPlusBoxDiv = rssPlusDocument.getElementById("rss-plus-box")
        if (!rssPlusBoxDiv){
            rssPlusBoxDiv = document.createElement("div");
...
function follow(link) {
    ...
    } else if (rssService === "follow") {
        window.open("follow://add?url=" + encodeURIComponent(link), "_blank");
    }
